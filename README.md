...
4. **「網羅性優先」か「引き算優先」かの判断軸**（`data/type_taxonomy.csv`の「情報方針」列）— サイトタイプごとに、情報を削らず全部見せるべきか、1つの見せ場に絞るべきかを切り分ける。店舗・施設紹介やEC・WEBサービスのような「情報を探しに来た人に判断材料を渡す」サイトでは、装飾面の引き算は行っても実務情報（価格・在庫・お知らせ・FAQ・連絡先等）は削らない。特設・キャンペーンサイトやポートフォリオのような「1つの訴求に絞る」サイトでは、引き算そのものが設計目標になる。既存サイトのリニューアルで情報が意図せず削られてしまう問題への対応として追加
...
| `data/type_taxonomy.csv` | サイトタイプ9分類のマスタ。「情報方針」列（網羅性優先／引き算優先／文脈依存）と判断メモつき |
