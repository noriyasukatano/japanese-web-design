...
### ステップ0: 新規制作かリニューアルかを見極める（最重要・毎回確認する）

既存サイト・既存デザインシステムに対してこのSkillを適用する場合（リニューアル・改修）は、**既存のセクション構成・コンテンツ要素を勝手に削らない**。

このSkillが提供するのは「業種の実測データに基づく作り方（配色・フォント・文字組み・moderate/boldの寄せ方）」であり、「既存サイトの内容を業種の典型パターンに合わせて簡略化する」ためのものではない。実測データに登場する項目数（例：サービス3〜4件、実績2〜3件）はあくまで母集団の傾向であり、既存サイトが持つ独自の情報量（料金表・FAQ・実績件数・訴求ポイントの数など）を削る根拠にはならない。

- 既存サイト・既存デザインシステムがある場合：まず既存のセクション一覧・コンテンツ量を棚卸しし、それを維持した状態を前提にする。要素を削る提案をしたい場合は、削る理由（例：情報が重複している、優先度が低い）を明示し、必ずユーザーに確認を取ってから行う。無言で削らない。
- 新規制作（ゼロから作る）場合：既存の縛りがないため、ステップ2以降のデータドリブンな判断を通常通り適用してよい。
...
**コピーを写真・ヒーロー画像の上に直接乗せる場合は、`scripts/hero_text_zone.py` で画像を解析してから配置する。** 縦組・横組を問わず、背景が写真のときに文字を機械的に固定位置へ置くと、被写体（車・人物・建物等）の上に重なって読みにくくなったり、コントラスト不足になったりする。

```bash
python3 scripts/hero_text_zone.py path/to/hero-image.jpg --orientation vertical --top 5
# 横組コピーの場合は --orientation horizontal
```

出力される候補エリア（位置%・推奨文字色・flatness_score）を参考に、情報密度が低く（busynessが低い）コントラストを確保しやすい場所にコピーを配置する。適したエリアが見つからない場合や、どうしても被写体と重なる位置にしか置けない場合は、その周辺だけに限定したグラデーションスクリム（例：`background: linear-gradient(...)`)を敷いて可読性を確保する。画像全体に一律のスクリムをかけるのではなく、文字が乗る範囲だけに絞るのが望ましい。
...
6. **既存要素が削られていないか（リニューアル時）**：ステップ0で棚卸しした既存のセクション・コンテンツ量と比べて、無断で削られた要素がないか確認する。
7. **ヒーロー画像上のテキストが被写体と衝突していないか**：写真の上にコピーを乗せた箇所は、`scripts/hero_text_zone.py`の解析結果通りの位置になっているか、被写体に文字が重なって読みにくくなっていないかを目視で確認する。
