...
このステップは「セクションを丸ごと削らない」というマクロの歯止め。仕上げ段階での装飾レベルの引き算については、ステップ2で特定するタイプの「情報方針」（網羅性優先か引き算優先か）に従う（ステップ7も参照）。
...
新規にページ全体をレイアウトするとき、またはステップ7の最終確認のときは、業種を問わない一般原則（近接・整列・強弱・反復、配色比率、タイポグラフィ基本、モーションの節度、UIライティング）をまとめた `references/general-design-principles.md` も参照する。スクロールに合わせて要素が「フワッと」出てくるアニメーション（フェードイン＋スライドアップ）を追加したいときも、同ファイルの「4. モーションは節度をもって」内の実装節（CSS/JSの具体例、実測の数値目安）を参照する。
...
**タイプを特定したら、`data/type_taxonomy.csv` の「情報方針」列を必ず確認する（最重要）。** この列は「網羅性優先」「引き算優先」「文脈依存」のいずれかで、そのサイトタイプが本質的に何を目的にしているかを表す。

- **網羅性優先**（例：店舗・施設紹介、EC・WEBサービス、メディア・ポータル）：情報を探しに来た人に判断材料（価格・在庫・実績・アクセス・FAQ等）を漏れなく見せることが、そのままデザインの目的。ステップ7の「引き算の仕上げ」チェックは装飾・強調表現のみに適用し、実務情報を対象にしない。
- **引き算優先**（例：特設・キャンペーンサイト、ポートフォリオ）：単一の目的・見せ場に絞ること自体が価値。1つの訴求に集中させ、余白を大きく取ってよい。
- **文脈依存**（例：コーポレートサイト、サービス紹介、採用サイト、商品・製品紹介）：業種・ブリーフの性質で判断する。単一ブランド・単一商品を情緒的に見せるLPなら引き算優先、複数事業・複数商品・複数プランを比較検討してもらうサイトなら網羅性優先に倒す。`data/type_taxonomy.csv`の「判断メモ」列に具体例がある。

「大胆（bold）」と「情報が少ない（引き算）」は別軸であることに注意する。網羅性優先のサイトでも、配色やタイポグラフィ、レイアウトの見せ方は大胆にできる（情報量を保ったまま大胆にする）。moderate/boldダイアル（ステップ4）は「スタイルの冒険度」の軸であり、「情報方針」は「情報量の軸」。両者を混同して、大胆にしようとして情報を削らない。
...
| `data/type_taxonomy.csv` | サイトタイプ（9分類）ごとの「情報方針」（網羅性優先／引き算優先／文脈依存）と判断メモ。ステップ2で必ず確認する。 |
...
`references/principles.md` の「4. moderate / bold ダイアル」に従い、ユーザーの指定またはブリーフの文脈から、どちらに寄せるかを判断する。指定がなければ業種の性質から妥当な位置を選ぶ。前述の通り、これは情報量を減らすかどうかの判断ではない。「網羅性優先」タイプでboldに寄せる場合は、情報量を保ったまま配色・タイポグラフィ・レイアウトを大胆にする方向で考える。
...
5. **大胆さの一点集中／引き算の範囲チェック**：装飾・モーション・強調がページ全体に均等にばらまかれていないか。ここで「最後に何か削れないか」と検討する対象は、ステップ2で確認した「情報方針」に従う。「引き算優先」タイプ、または「文脈依存」タイプでブランド訴求に倒すと決めた場合のみ、ブリーフに貢献していない装飾を一つ削れないか検討する。「網羅性優先」タイプ（店舗・施設紹介、EC・WEBサービス、メディア・ポータル等）では、この引き算チェックの対象を純粋な装飾・強調表現に限定し、価格・在庫・お知らせ・FAQ・連絡先などの実務情報には適用しない。
...
8. **アニメーション（スクロール連動フェードイン等）が節度を保っているか**：`references/general-design-principles.md`「4. モーションは節度をもって」の実装節に沿っているか。具体的には、(a) スクロールで要素が画面を出入りするたびに何度も明滅していないか（`IntersectionObserver`で一度きり発火させているか）、(b) `prefers-reduced-motion`環境で動きが止まり、情報自体は問題なく見えるか、(c) 全セクションに同じ演出を機械的に付けて単調になっていないか、を確認する。
